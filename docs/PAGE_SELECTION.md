...
`page_numbers` is an ascending tuple of unique, positive, one-based source page numbers. Invalid, duplicate, descending, empty, or out-of-range selections fail before publication. Atomic output handling therefore preserves the previous known-good artifact.
